...
func main() {
	...
	for i, line := range util.ReadInputLines("./input.txt") {
		if i == 0 {
			...
			boardBuild = &Board{RemainingScore: 0, Cells: [][]int{}, HasWon: false}
		} else {
			cells := []int{}
	...
	for _, call := range calls {
		...
		for _, ref := range refs {
			...
			if ref.Board.call(ref.X, ref.Y) {
				// WIN!
				ref.Board.HasWon = true

				// Could store in variables and only output the actual answer, but I'm lazy so just pick the last printed answer :)
